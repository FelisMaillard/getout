    ...
    <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 md:pt-0">
        <div class="bg-black rounded-lg border border-gray-800 p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-white">Modifier le serveur</h1>
                <form action="{{ route('servers.destroy', $server) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                            onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce serveur ? Cette action est irréversible.')"
            ...
            <form action="{{ route('servers.update', $server) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description"
                              id="description"
                              rows="3"
                              class="w-full px-4 py-2 bg-gray-900 border border-gray-800 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent">{{ old('description', $server->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="space-y-4">
                        <label class="flex items-center">
                            <input type="radio"
                                   ...
                                   {{ old('privacy_type', $server->privacy_type) === 'public' ? 'checked' : '' }}>
                        ...
                        <label class="flex items-center">
                            <input type="radio"
                                   ...
                                   value="private"
                                   class="w-4 h-4 text-purple-600 border-gray-800 focus:ring-purple-600 focus:ring-offset-gray-900"
                                   {{ old('privacy_type', $server->privacy_type) === 'private' ? 'checked' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <input type="number"
                           ...
                           max="1000"
                           class="w-full px-4 py-2 bg-gray-900 border border-gray-800 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                           value="{{ old('max_members', $server->max_members) }}"
                           required>
